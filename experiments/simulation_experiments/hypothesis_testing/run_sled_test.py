...
if __name__ == "__main__":
    ...
    for ii, n_genes in enumerate(p_list):
        ...
        for jj in range(NUM_REPEATS):
            ...
            os.system("Rscript sled_test.R")
            curr_output = pd.read_csv("./tmp/curr_sled_output.csv", index_col=0)
            # import ipdb; ipdb.set_trace()
            test_stats_shuffled.append(curr_output.Tn.values[0])
            decisions_shuffled.append(curr_output.pVal.values[0] < alpha)

            for ff in os.listdir("./tmp"):
                os.remove(os.path.join("tmp", ff))

        np.save(
            "../out/sled/test_stats_experiment_p{}.npy".format(n_genes),
            test_stats_experiment,
        )

        np.save(
            "../out/sled/test_stats_shuffled_p{}.npy".format(n_genes),
            test_stats_shuffled,
        )
