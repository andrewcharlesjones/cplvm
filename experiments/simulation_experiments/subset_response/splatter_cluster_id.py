...
if __name__ == "__main__":
    ...
    fg_labels_strings = np.array(
        [
            "FG (unresponsive)" if fg_labels[i] == 0 else "FG (responsive)"
    ...
    sns.scatterplot(
        data=z_df, x="z1", y="z2", style="label", markers=["*", "o", "v"], color="black"
    )
    plt.xlabel(r"$z_1$")
    plt.ylabel(r"$z_2$")
    ax = plt.gca()
    ...
    sns.scatterplot(
        data=ty_df, x="ty1", y="ty2", style="label", markers=["o", "v"], color="black"
    )
    plt.xlabel(r"$t_1$")
    plt.ylabel(r"$t_2$")
    ax = plt.gca()
