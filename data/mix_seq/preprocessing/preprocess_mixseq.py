...
barcodes_dmso = pd.read_table(
    pjoin(DATA_DIR, "DMSO_24hr_expt1/barcodes.tsv"), header=None
)
classifications_dmso = pd.read_csv(
    pjoin(DATA_DIR, "DMSO_24hr_expt1/classifications.csv")
)
classifications_dmso["cell_line"] = np.array(
    [x.split("_")[0] for x in classifications_dmso.singlet_ID.values]
)
gene_names_dmso = pd.read_table(
    pjoin(DATA_DIR, "DMSO_24hr_expt1/genes.tsv"), header=None
)
data_dense_dmso = pd.DataFrame(
    data_dmso.toarray(),
    columns=barcodes_dmso.iloc[:, 0].values,
    index=gene_names_dmso.iloc[:, 0].values,
)


# Nutlin
data_nutlin = mmread(pjoin(DATA_DIR, "Idasanutlin_24hr_expt1/matrix.mtx"))
barcodes_nutlin = pd.read_table(
    pjoin(DATA_DIR, "Idasanutlin_24hr_expt1/barcodes.tsv"), header=None
)
classifications_nutlin = pd.read_csv(
    pjoin(DATA_DIR, "Idasanutlin_24hr_expt1/classifications.csv")
)
classifications_nutlin["cell_line"] = np.array(
    [x.split("_")[0] for x in classifications_nutlin.singlet_ID.values]
)
gene_names_nutlin = pd.read_table(
    pjoin(DATA_DIR, "Idasanutlin_24hr_expt1/genes.tsv"), header=None
)
data_dense_nutlin = pd.DataFrame(
    data_nutlin.toarray(),
    columns=barcodes_nutlin.iloc[:, 0].values,
    index=gene_names_nutlin.iloc[:, 0].values,
)

print(
    "Loaded {} DMSO cells and {} nutlin cells".format(
        data_dense_dmso.shape[1], data_dense_nutlin.shape[1]
    )
)
...
mutation_data = pd.read_csv(
    "/Users/andrewjones/Downloads/TP53 Gene Effect (CERES) CRISPR (Avana) Public 20Q3.csv"
)
...
for curr_cl in classifications_nutlin.cell_line.values:
    if curr_cl not in mutation_data["Cell Line Name"].unique():
        ...
        p53_mutations.append(
            mutation_data[mutation_data["Cell Line Name"] == curr_cl][
                "TP53 Mutations"
            ].values[0]
        )
...
p53_mutations_df = pd.DataFrame(
    p53_mutations, index=data_dense_nutlin_variable.index.values
)
...
p53_mutations_df = pd.DataFrame(
    p53_mutations, index=data_dense_dmso_variable.index.values
)
p53_mutations_df.columns = ["tp53_mutation"]
