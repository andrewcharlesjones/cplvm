...
v8_metadata_sample = pd.read_table(
    METADATA_PATH_SAMPLE)
v8_metadata_sample['sample_id'] = [
    '-'.join(x.split("-")[:3]) for x in v8_metadata_sample.SAMPID.values]

# Subject-level metadata
v8_metadata_subject = pd.read_table(
    METADATA_PATH_SUBJECT)

# get lung tissue IDs
heart_sample_ids = v8_metadata_sample.sample_id.values[v8_metadata_sample.SMTSD == "Artery - Coronary"]
...
noheartdisease_ids = v8_metadata_subject.SUBJID.values[v8_metadata_subject.MHHRTDIS == 0]


# Get sample names of expression data
expression_ids = pd.read_table(EXPRESSION_PATH, skiprows=2, index_col=0, nrows=1).columns.values[1:]
...
heartdisease_idx = np.where(np.logical_and(np.isin(expression_sample_ids, heart_sample_ids), np.isin(expression_subject_ids, heartdisease_ids)) == True)[0]
noheartdisease_idx = np.where(np.logical_and(np.isin(expression_sample_ids, heart_sample_ids), np.isin(expression_subject_ids, noheartdisease_ids)) == True)[0]
assert len(np.intersect1d(heartdisease_idx, noheartdisease_idx)) == 0

# Load expression
expression_heartdisease = pd.read_table(EXPRESSION_PATH, skiprows=2, index_col=0, usecols=np.insert(heartdisease_idx, 0, 0))
expression_noheartdisease = pd.read_table(EXPRESSION_PATH, skiprows=2, index_col=0, usecols=np.insert(noheartdisease_idx, 0, 0))
