...
def cai_test(X1, X2, alpha=0.05, verbose=False):
	...
	for ii in range(p):
		...
		theta1[ii, :] = 1 / n1 * np.sum((np.multiply(np.expand_dims(X1_centered[:, ii], axis=1), X1_centered) - sigma1[ii, :])**2, axis=0)
		theta2[ii, :] = 1 / n2 * np.sum((np.multiply(np.expand_dims(X2_centered[:, ii], axis=1), X2_centered) - sigma2[ii, :])**2, axis=0)
...
def generate_random_cov(p):
	...
	for ii in range(p):
		for jj in range(p):
			Delta[ii, jj] = np.power(-1, ii + 1 + jj + 1) * np.power(0.4, np.abs(ii + 1 - jj + 1)**0.1)
