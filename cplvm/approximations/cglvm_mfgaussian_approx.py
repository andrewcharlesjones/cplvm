...
class CGLVMMFGaussianApprox(ApproximateModel):
	...
	def __init__(self, X, Y, k_shared, k_foreground, num_test_genes=0, is_H0=False):
		...
		self.qw_mean = tf.Variable(tf.random.normal([self.data_dim, self._k_foreground]))
		...
		self.qw_stddv = tfp.util.TransformedVariable(
		  1e-4 * tf.ones([self.data_dim, self._k_foreground]),
		  bijector=tfb.Softplus())
		...
		def factored_normal_variational_model():
		  qmu_x = yield tfd.Normal(loc=self.qmu_x_mean, scale=self.qmu_x_stddv, name="qmu_x")
		  qmu_y = yield tfd.Normal(loc=self.qmu_y_mean, scale=self.qmu_y_stddv, name="qmu_y")
		  ...
		  if not is_H0:
		  	...
		  	qty = yield tfd.Normal(loc=self.qty_mean, scale=self.qty_stddv, name="qty")
