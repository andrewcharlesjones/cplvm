...
def clvm(data_dim, latent_dim_shared, latent_dim_target, num_datapoints_x, num_datapoints_y, counts_per_cell_X, counts_per_cell_Y, is_H0=False):
    ...
    if is_H0:
        ...
        x = yield tfd.Gaussian(loc=tf.math.add(tf.matmul(s, zx), mu_x),
                            scale=tf.ones([data_dim, num_datapoints_x]),
                         name="x")

        y = yield tfd.Gaussian(loc=tf.math.add(tf.matmul(s, zy), mu_y),
        ...
        x = yield tfd.Normal(loc=tf.math.add(tf.matmul(s, zx), mu_x),
                            scale=tf.ones([data_dim, num_datapoints_x]),
                         name="x")

        y = yield tfd.Normal(loc=tf.math.add(tf.matmul(s, zy) + tf.matmul(w, ty), mu_y),
                            scale=tf.ones([data_dim, num_datapoints_y]),
                             name="y")


def fit_model(X, Y, latent_dim_shared, latent_dim_target, compute_size_factors = True, is_H0 = False):
    ...
    if is_H0:
        ...
        def target_log_prob_fn(deltax, deltay, s, zx, zy): return model.log_prob(
            (deltax, deltay, s, zx, zy, X, Y))

    else:

        def target_log_prob_fn(deltax, deltay, s, zx, zy, w, ty): return model.log_prob(
            (deltax, deltay, s, zx, zy, w, ty, X, Y))
