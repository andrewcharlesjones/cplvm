...
def pca(data_dim, latent_dim, num_datapoints, counts_per_cell):
    ...
    s = yield tfd.Gamma(concentration=tf.ones([data_dim, latent_dim]),
                 rate=tf.ones([data_dim, latent_dim]),
                 name="w")

    zx = yield tfd.Gamma(concentration=tf.ones([latent_dim, num_datapoints]),
                   rate=tf.math.multiply(tf.ones([latent_dim, num_datapoints]), 1/counts_per_cell),
                   name="zx")

    x = yield tfd.Poisson(rate=tf.matmul(s, zx),
                    name="x")
...
def fit_model(X, latent_dim, compute_size_factors = True):
    ...
    concrete_clvm_model=functools.partial(pca,
                                            data_dim = data_dim,
    ...
    def target_log_prob_fn(s, zx): return model.log_prob(
        (s, zx, X))
    ...
    qs_stddv = tfp.util.TransformedVariable(
      1e-4 * tf.ones([data_dim, latent_dim]),
