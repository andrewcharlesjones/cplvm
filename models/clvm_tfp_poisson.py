...
def clvm(data_dim, latent_dim_shared, latent_dim_target, num_datapoints_x, num_datapoints_y, counts_per_cell_X, counts_per_cell_Y, is_H0=False, num_test_genes=0, offset_term=True):
	...
	if offset_term:
		...
		# deltax = yield tfd.Gamma(concentration=tf.ones([data_dim, 1]),
		# 		   rate=tf.ones([data_dim, 1]),
		# 		   name="deltax")
	else:
	...
	zx = yield tfd.Gamma(concentration=tf.ones([latent_dim_shared, num_datapoints_x]),
				   rate=tf.math.multiply(5 * tf.ones([latent_dim_shared, num_datapoints_x]), 1),
				   name="zx")

	zy = yield tfd.Gamma(concentration=tf.ones([latent_dim_shared, num_datapoints_y]),
				   rate=tf.math.multiply(5 * tf.ones([latent_dim_shared, num_datapoints_y]), 1),
				   name="zy")
	...
	if is_H0:
		...
		ty = yield tfd.Gamma(concentration=tf.ones([latent_dim_target, num_datapoints_y]),
				   rate=tf.math.multiply(5 * tf.ones([latent_dim_target, num_datapoints_y]), 1),
				   name="ty")
...
def fit_model(X, Y, latent_dim_shared, latent_dim_target, compute_size_factors = True, is_H0 = False, num_test_genes=0, offset_term=True):
...
def fit_model_map(X, Y, latent_dim_shared, latent_dim_target, compute_size_factors = True, is_H0 = False, num_test_genes=0, offset_term=True):
